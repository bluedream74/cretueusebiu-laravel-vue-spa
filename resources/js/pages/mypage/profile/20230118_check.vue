			...
			<div class="content_area form_wrap check">
				<div>
					<section class="section lead">
						<div class="sitewrap">
							<h2 class="h_style01">マイプロフィール変更確認</h2>
							<p class="lead_style">内容をご確認の上、登録ボタンを押してください。</p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async register() {
        try {
          const { data } = await axios.post('/api/update_profile_info', {
          ...
          this.clearContent()
          this.$router.push({ name: 'profile.thanks' })
        } catch (error) {
        }
      },
